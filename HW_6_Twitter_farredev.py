...
def construct_unique_key(baseurl, params):
    ...
    for k in params.keys():
        parameter.append(f'{k}_{params[k]}')
    parameter.sort() 
    unique_key = baseurl + underscore + underscore.join(parameter)
    return unique_key   
...
def make_request(baseurl, params):
    ...
    dict
        ...
        a dictionary
    '''
    results = requests.get(url=baseurl, params=params, auth=oauth)
    results_new = results.json()
    return results_new
...
def make_request_with_cache(baseurl, hashtag, count):
    ...
    if request in CACHE_DICTION.keys():
        print('fetching cached data')
        return CACHE_DICTION[request]
    else:
        print("making new request")
        CACHE_DICTION[request]  = make_request(baseurl, params)
...
def find_most_common_cooccurring_hashtag(tweet_data, hashtag_to_ignore):
    ...
    for items in tweet_data.values():
        tweet_data_list.append(items)
        status = tweet_data_list[0]
        for x in status:
            for item in x['entities']['hashtags']:
                temp_string = '#' + item['text'].lower()
                ignore_string = hashtag_to_ignore.lower()  
                if temp_string != ignore_string:
                    hashtag_list.append(item['text'].lower())
